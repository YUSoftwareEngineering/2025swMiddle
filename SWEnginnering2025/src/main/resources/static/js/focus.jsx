...
// 세션 저장소 관리 (localStorage 기반)
const focusStorage = {
    KEY: 'focusSessions',

    getSessions: () => {
        try {
            const data = localStorage.getItem(focusStorage.KEY);
    ...
    addSession: (session) => {
        ...
        localStorage.setItem(focusStorage.KEY, JSON.stringify(trimmed));
    },

    getCurrentSession: () => {
        try {
            const data = localStorage.getItem('currentFocusSession');
